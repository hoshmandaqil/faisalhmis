...
@section('page_title')
    Print and Get Pay from Patients
@endsection
        ...
        <div class="row justify-content-center">
            <div class="col-xl-5 col-lg-6 col-md-7 col-sm-8 col-12">
                ...
                <div class="search-box">
                    <form action="{{url('search_reception_medicine_patient')}}" method="post">
                        @csrf
                        <input type="text" name="search_patient" class="search-query"
                               value="{{(Request::is('search_reception_medicine_patient') ? $patientSearchDetail : '')}}" placeholder="Search Patient By Id, Name or Phone...">
...
@endsection
@section('page-action')
    @if(\Request::is('search_reception_medicine_patient'))
        <a type="button" class="btn btn-danger btn-sm" href="{{url('patient_reception_medicine')}}">
            Clear Search
        </a>
    @endif
@endsection
                ...
                <table id="scrollVertical" class="table">
                    ...
                    @foreach($medicinePatients  as $patient)
                        ...
                        <tr>
                            ...
                            <td>
                                @foreach($patient->pharmacyMedicines as $pharmacyMedicine)
                                    <?php $grandTotal += $pharmacyMedicine->quantity * $pharmacyMedicine->unit_price?>
                                    @endforeach
                            {{$grandTotal}}
                            <td>

                                @if($patient->pharmacyMedicines()->exists())
                                    @if(in_array('reception_print_medicine', $user_permissions))
                                        <button class="btn btn-sm btn-secondary" data-patient-id="{{$patient->id}}"
                                                data-patient-name="{{$patient->patient_name}}"
                                                onclick="printPreviewMedicine({{$patient->id}})">Print <i
                                                class="icon icon-print"></i></button>

                                    @endif

                                    @if(in_array('reception_preview_medicine', $user_permissions))
        ...
        <div class="modal-dialog modal-lg " role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Sell Medicine To <span id="sell_medicine_patient_name"></span></h5>
...
@section('scripts')
    <script>
        ...
        function printPreviewMedicine(id) {
            ...
            if (id != '') {
                ...
                printExternal('{{ url("previewPatientMedicines") }}'+"?patient_id="+id);
            }
        }
