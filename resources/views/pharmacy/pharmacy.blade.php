                ...
                <div class="search-box">
                    <form action="{{ url('search_medicine') }}" method="GET">
                        <input type="text" name="search_medicine" class="search-query"
                            value="{{ Request::is('search_medicine') ? $medicineSearchDetail : '' }}"
                            placeholder="Search Medicine By Name, QTY, Invoice, Remark or Price">
                    ...
                    <tbody>
                        @foreach ($pharmacies as $pharmacy)
                            <tr>
                                ...
                                <td>{{ $pharmacy->supplier != null ? $pharmacy->supplier->supplier_name : 'No Supplier' }}
                                </td>
                                            ...
                                            <div class="btn-group" role="group">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                                    <a class="dropdown-item text-info" href="#" data-toggle="modal"
                                                        ...
                                                        data-medicine-id="{{ $pharmacy->medicine_id }}"
                                                        data-supplier="{{ $pharmacy->supplier_id }}"
                                                        data-quantity="{{ $pharmacy->quantity }}"
                                                        data-purchase-price="{{ $pharmacy->purchase_price }}"
                                                        data-percentage="{{ $pharmacy->sale_percentage }}"
                                                        data-sale-price="{{ $pharmacy->sale_price }}"
                                                        data-remark="{{ $pharmacy->remark }}"
                                                        data-invoice="{{ $pharmacy->invoice_no }}"
                                                    ...
                                                    @if ($pharmacy->returned == 0)
                                                        <a class="dropdown-item text-warning"
                                                            href="{{ url('return_medicine', $pharmacy->id) }}"
                                                            onclick="return confirm('Are you sure You want to return this Medicine?')"><i
                                                                class="icon icon-block"></i> Return Medicine</a>
                                                    @else
                                                        <a class="dropdown-item text-warning"
                                                            href="{{ url('undo_return_medicine', $pharmacy->id) }}"
                                                            onclick="return confirm('Are you sure You want to Undo return this Medicine?')"><i
                                                                class="icon icon-block"></i> Undo Return</a>
                                                    @endif

                                                    @if ($pharmacy->expired == 0)
                                                        <a class="dropdown-item text-success"
                                                            href="{{ url('expire_this_medicine', $pharmacy->id) }}"
                                                            onclick="return confirm('Are you sure You want to expire this Medicine?')"><i
                                                                class="icon icon-calendar"></i> Expire Medicine</a>
                                                    @else
                                                        <a class="dropdown-item text-success"
                                                            href="{{ url('undo_expire_this_medicine', $pharmacy->id) }}"
                                                            onclick="return confirm('Are you sure You want to Undo expire this Medicine?')"><i
                                                                class="icon icon-calendar"></i> Undo Expire</a>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
